...
class ImageUploader {
  ...
  public function upload() {
    ...
    // redirect
    // header('Location: http://' . $_SERVER['HTTP_HOST'] . '/imgloader/');
  ...
  private function _save($ext) {
    ...
    if ($res === false) {
      throw new \Exception('アップロード失敗しました');
    }
    return $savePath;
  }
  ...
  private function _validateImageType() {
    ...
    switch($this->_imageType) {
      ...
      case IMAGETYPE_PNG:
        return 'png';
      default:
        throw new \Exception('PNG / JPEG / GIF 形式でアップロードしてください');
    }
  }
  ...
  private function _validateUpload() {
    ...
    switch($_FILES['image']['error']) {
      ...
      case UPLOAD_ERR_FORM_SIZE:
        throw new \Exception('FILE too large!(MAX SIZE: 1MB)');
      default:
